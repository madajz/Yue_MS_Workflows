...
data = read.csv("~/bbc-secondary/research/MOOD_yue_MS_template_VBCS1110/liver_metabolimcs/MC00331_liver_combined metabolomics.csv",header=T)
...
data$Geno =  sapply(strsplit(data$Sample,"_"), function(x) x[1])
...
if(length(unique(data$Geno ))>2) {
  ...
  f.df =  data.frame(Contrast = "KI/KI - WT/WT", topTable(ebayesf, adjust.method = "BH", p.value = 1, number = Inf,coef=1))
...
ggplot(f.df,aes(x=logFC,y=-log10(P.Value),color=Significant)) + geom_point()+
  theme_bw(16) +facet_wrap(~Contrast)+geom_text_repel(data = f.df[f.df$label!= "",] ,aes(label=Compound))+scale_color_manual(values = c("NS"="black","Significant" = "red")) + ggtitle("KI - WT")
...
if(length(unique(data$Geno ))>2) {
  ...
  m.df =  data.frame(Contrast = "KI/KI - WT/WT", topTable(ebayesf, adjust.method = "BH", p.value = 1, number = Inf,coef=2))
...
if(length(unique(data$Geno)) > 2){
  ...
  pool.df = data.frame(Contrast = "KI/KI - WT/WT", topTable(ebayesf, adjust.method = "BH", p.value = 1, number = Inf,coef=1))
